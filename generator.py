...
# A sentence's audio is generated by its own generate_stream() call, with the
# decoder's streaming state reset in between, so consecutive sentences have no
# waveform continuity at all: sentence N can end mid-cycle at a large amplitude
# and sentence N+1 starts from wherever its own waveform happens to begin. Once
# the client splices them (or just butts one against silence) that step is an
# audible click. Chunk boundaries *within* a sentence are continuous and must
# NOT be faded - doing that would notch the waveform every buffer_size frames.
EDGE_FADE_MS = 5.0


def _apply_edge_fade(audio: torch.Tensor, sample_rate: int, fade_in: bool = False, fade_out: bool = False):
    """Raised-cosine ramp on the outer edges of a sentence's audio, in place."""
    if audio is None or audio.numel() == 0:
        return audio

    n = min(int(sample_rate * EDGE_FADE_MS / 1000.0), audio.shape[-1])
    if n <= 1:
        return audio

    # 0 -> 1 over n samples, smooth at both ends (no derivative jump of its own).
    ramp = 0.5 - 0.5 * torch.cos(torch.linspace(0, math.pi, n, dtype=audio.dtype, device=audio.device))
    if fade_in:
        audio[:n] *= ramp
    if fade_out:
        audio[-n:] *= torch.flip(ramp, dims=[0])
    return audio


def _split_carry(chunk: torch.Tensor, carry: torch.Tensor, sample_rate: int):
    """Emit `carry + chunk` minus its trailing fade-length, holding that back.

    Costs EDGE_FADE_MS of extra latency (5ms) and keeps the stream sample-exact:
    nothing is dropped, it just crosses the wire one chunk later.
    """
    n = int(sample_rate * EDGE_FADE_MS / 1000.0)
    combined = torch.cat([carry, chunk]) if carry.numel() else chunk
    if combined.shape[-1] <= n:
        return combined.new_empty(0), combined.clone()
    return combined[:-n].clone(), combined[-n:].clone()
...
class Generator:
    ...
    def generate_stream(
        ...
        frame_buffer = []
        # Last few ms of decoded audio, withheld from the previous yield so the
        # sentence's final samples are always ours to fade out (see _split_carry).
        carry = torch.zeros(0)
        ...
        with self._audio_tokenizer.streaming(1):
            ...
            while i < max_generation_len:
                ...
                if len(frame_buffer) >= buffer_size:
                    ...
                    # Hold back the last few ms so that whichever chunk turns
                    # out to be the sentence's last one, we still own its final
                    # samples and can ramp them down. Without this, a sentence
                    # whose frame count lands on an exact multiple of the buffer
                    # size (or whose EOS falls on a batch boundary) would end on
                    # an already-sent chunk, cut off at full amplitude.
                    cpu_chunk, carry = _split_carry(cpu_chunk, carry, self.sample_rate)
                    if cpu_chunk.numel() == 0:
                        continue

                    # Only the sentence's very first chunk gets a fade-in: it is
                    # the one edge that abuts either silence or the previous
                    # sentence's tail. Later chunks continue the same waveform.
                    if not first_chunk_delivered:
                        _apply_edge_fade(cpu_chunk, self.sample_rate, fade_in=True)
            ...
            if frame_buffer:
                ...
                cpu_chunk = audio_chunk.cpu().clone()
                if carry.numel():
                    cpu_chunk = torch.cat([carry, cpu_chunk])
                    carry = carry.new_empty(0)
            else:
                # Loop ended on a chunk boundary with nothing buffered; the only
                # audio still owed is the held-back tail.
                cpu_chunk = carry
                carry = carry.new_empty(0)

            if cpu_chunk.numel():
                # This is the sentence's last chunk, and the padding trim above
                # (plus a cap-hit truncation mid-word) cuts the waveform at an
                # arbitrary amplitude. Ramp it down so the transition into
                # silence or into the next sentence isn't a step. Fade in too if
                # this is also the first chunk - a sentence short enough to
                # never fill the buffer.
                _apply_edge_fade(
                    cpu_chunk, self.sample_rate,
                    fade_in=not first_chunk_delivered,
                    fade_out=True,
                )
